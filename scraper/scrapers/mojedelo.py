"""
MojeDelo.com scraper (Slovenija) - Hybrid/Onsite jobs
"""
from crawlee_playwright import (
    PlaywrightCrawler,
    PlaywrightCrawlingContext
)
from datetime import datetime
from typing import List, Dict, Any
from utils.logger import Logger
from utils.normalizer import (
    calculate_quality_score,
    clean_text,
    extract_salary
)

logger = Logger("mojedelo")


async def scrape_mojedelo(max_jobs: int = 50) -> List[Dict[str, Any]]:
    """
    Scrape hybrid/onsite jobs from MojeDelo.com (Slovenia)
    
    Args:
        max_jobs: Maximum number of jobs to scrape
    
    Returns:
        List of job dictionaries
    """
    jobs = []
    
    async def request_handler(context: PlaywrightCrawlingContext) -> None:
        """Handle each page request."""
        logger.info(f"Scraping {context.request.url}")
        
        page = context.page
        
        # Wait for job listings
        try:
            await page.wait_for_selector('.job-list-item', timeout=10000)
        except Exception as e:
            logger.error("Failed to load job listings", e)
            return
        
        # Extract job cards
        job_cards = await page.query_selector_all('.job-list-item')
        
        for card in job_cards[:max_jobs]:
            if len(jobs) >= max_jobs:
                break
            
            try:
                # Title
                title_el = await card.query_selector('.job-title a')
                if not title_el:
                    continue
                
                title = clean_text(await title_el.inner_text())
                url = await title_el.get_attribute('href')
                
                # Company
                company_el = await card.query_selector('.company-name')
                company = clean_text(
                    await company_el.inner_text()
                ) if company_el else "Unknown Company"
                
                # Location
                location_el = await card.query_selector('.job-location')
                location = clean_text(
                    await location_el.inner_text()
                ) if location_el else "Ljubljana, Slovenija"
                
                # Salary
                salary_el = await card.query_selector('.salary-info')
                salary_min, salary_max, currency = None, None, 'EUR'
                if salary_el:
                    salary_text = await salary_el.inner_text()
                    salary_min, salary_max, currency = extract_salary(
                        salary_text
                    )
                
                if not title or not url:
                    continue
                
                # Generate external_id
                job_id = url.split('/')[-1].split('?')[0]
                external_id = f"mojedelo-{job_id}"
                
                # Create job object
                job = {
                    'external_id': external_id,
                    'source_name': 'MojeDelo.com',
                    'title': title,
                    'company_name': company,
                    'location': location,
                    'work_type': 'hybrid',
                    'country_code': 'SI',
                    'region': 'BALKAN',
                    'category': 'software-engineering',
                    'description': f"{title} pri {company}",
                    'application_url': (
                        f"https://www.mojedelo.com{url}"
                        if url.startswith('/')
                        else url
                    ),
                    'source_website': 'https://www.mojedelo.com',
                    'experience_level': 'mid',
                    'employment_type': 'full-time',
                    'salary_min': salary_min,
                    'salary_max': salary_max,
                    'salary_currency': currency,
                    'posted_date': datetime.now().isoformat(),
                    'scraped_at': datetime.now().isoformat(),
                }
                
                # Calculate quality score
                job['quality_score'] = calculate_quality_score(job)
                
                jobs.append(job)
                logger.info(f"Scraped: {title} at {company}")
                
            except Exception as e:
                logger.error("Failed to parse job card", e)
                continue
    
    # Create crawler
    crawler = PlaywrightCrawler(
        request_handler=request_handler,
        headless=True,
        max_requests_per_crawl=5,
    )
    
    # Start scraping
    start_url = "https://www.mojedelo.com/prosta-delovna-mesta"
    
    try:
        logger.info(f"Starting MojeDelo scraper (max: {max_jobs} jobs)")
        await crawler.run([start_url])
        logger.success(f"MojeDelo completed: {len(jobs)} jobs")
    except Exception as e:
        logger.error("MojeDelo scraper failed", e)
    
    return jobs
