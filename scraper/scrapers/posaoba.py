...
logger = Logger("posaoba")


async def scrape_posaoba(max_jobs: int = 50) -> List[Dict[str, Any]]:
    """
    Scrape hybrid/onsite jobs from Posao.ba (Bosnia)
    ...
    async def request_handler(context: PlaywrightCrawlingContext) -> None:
        ...
        for card in job_cards[:max_jobs]:
            ...
            try:
                ...
                # Company
                company_el = await card.query_selector('.employer-name')
                company = clean_text(
                ...
                ) if location_el else "Sarajevo, BiH"
                
                # Salary
                salary_el = await card.query_selector('.salary')
                salary_min, salary_max, currency = None, None, 'BAM'
                if salary_el:
                ...
                job = {
                    ...
                    'application_url': (
                        f"https://www.posao.ba{url}"
                        if url.startswith('/')
                        else url
                    ),
                    'source_website': 'https://www.posao.ba',
    ...
    # Start scraping
    start_url = "https://www.posao.ba/poslovi"
    
    try:
        logger.info(f"Starting Posao.ba scraper (max: {max_jobs} jobs)")
        await crawler.run([start_url])
        logger.success(f"Posao.ba completed: {len(jobs)} jobs")
    except Exception as e:
