...
// Mirror of /api/scraper/run, but under /api/portal-jobs which routes OK in prod
export async function GET(req: NextRequest) {
  try {
    ...
    logger.error('Failed to start scrape (GET portal-jobs/scrape-all)', error)
...
export async function POST(req: NextRequest) {
  try {
    ...
    logger.error('Failed to start scrape (POST portal-jobs/scrape-all)', error)
    return NextResponse.json({ success: false, error: 'Failed to start scrape' }, { status: 500 })
  }
}
