...
import { BookOpen, Users, Monitor, CheckCircle, ArrowRight, Globe, AlertCircle, FileText, CreditCard, MapPin, Clock, Shield, Briefcase, Target } from 'lucide-react';
...
interface Category {
  ...
  name: string;
  icon: LucideIcon;
  color: string; // tailwind color key (just label use)
}
        ...
        <div className="mb-8 grid gap-4 md:grid-cols-[260px_1fr]">
          <aside className="bg-white rounded-xl border border-gray-200 p-4 h-fit sticky top-4 self-start">
            <button
              ...
              className="w-full text-left text-sm font-semibold text-gray-900 mb-3"
            >
              Sadržaj
            </button>
            {tocOpen && (
              <nav className="text-sm text-gray-700 space-y-1">
                <a href="#quick-start" className="block hover:text-blue-600">🚀 Quick Start</a>
                <a href="#tips" className="block hover:text-blue-600">📚 Saveti po kategorijama</a>
                <a href="#mistakes" className="block hover:text-blue-600">⚠️ Česte greške</a>
                <a href="#resources" className="block hover:text-blue-600">🔗 Korisni resursi</a>
              </nav>
            )}
          </aside>
          <div>
