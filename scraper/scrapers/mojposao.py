...
async def scrape_mojposao(max_jobs: int = 50) -> List[Dict[str, Any]]:
    """
    Scrape hybrid/onsite jobs from MojPosao.net (Croatia)
    ...
    async def request_handler(context: PlaywrightCrawlingContext) -> None:
        ...
        for card in job_cards[:max_jobs]:
            ...
            try:
                ...
                ) if location_el else "Zagreb, Hrvatska"
                
                # Salary
                salary_el = await card.query_selector('.job-salary')
                salary_min, salary_max, currency = None, None, 'EUR'
                if salary_el:
                ...
                if type_el:
                    ...
                    if 'remote' in type_text.lower():
                        work_type = 'remote'
                    elif 'ured' in type_text.lower():
                        work_type = 'onsite'
                ...
                external_id = f"mojposao-{job_id}"
                
                # Create job object
                job = {
                    'external_id': external_id,
                    'source_name': 'MojPosao.net',
                    'title': title,
                    ...
                    'description': f"{title} u {company}.",
                    'application_url': (
                        f"https://www.mojposao.net{url}"
                        if url.startswith('/')
                        else url
                    ),
                    'source_website': 'https://www.mojposao.net',
    ...
    # Start scraping
    start_url = "https://www.mojposao.net/posao"
    
    try:
        logger.info(f"Starting MojPosao scraper (max: {max_jobs} jobs)")
        await crawler.run([start_url])
        logger.success(f"MojPosao completed: {len(jobs)} jobs")
    except Exception as e:
